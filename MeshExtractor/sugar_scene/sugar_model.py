...
class SuGaR(nn.Module):
    ...
    def __init__(
        ...
        *args, **kwargs) -> None:
        ...
        if surface_mesh_to_bind is not None:
            ...
            self._points = nn.Parameter(points, requires_grad=self.learn_positions).to(nerfmodel.device)
            n_points = len(self._points) ## 143080
        ...
        self._types = nerfmodel.gaussians.get_type
        self._normals = nerfmodel.gaussians.get_normals
    ...
    def get_normals(self, estimate_from_points=False,Mask = None  ,neighborhood_size:int=32):
        ...
        if estimate_from_points:
            normals = estimate_pointcloud_normals(
                self.points[None], #.detach(),   ### 为什么self.points和
            ...
            if self.binded_to_surface_mesh:
                ...
                normals = self.get_smallest_axis()  ### exe
        if Mask is not None :
            normals_origin = self.nerfmodel.gaussians.get_rotation ### 有输出的  rotation
            normals_origin = quaternion_to_matrix(normals_origin)
            smallest_axis_idx = self.nerfmodel.gaussians.get_opacity.min(dim=-1)[1][..., None, None].expand(-1, 3, -1)
            smallest_axis = normals_origin.gather(2, smallest_axis_idx)
            smallest_axis = smallest_axis.squeeze(dim=2)
            normals_new = smallest_axis[~Mask]
            types_origin = self.nerfmodel.gaussians.get_type  ### 有输出的  1表示扁平球 0表示正常的
            print("The numbe of types:")
            print("The numbe of types:")
            print("The numbe of types:")
            print("The numbe of types:")
            print("The numbe of types:")
            print("The numbe of types:")
            print("The numbe of types:", types_origin.shape)
            types_new = types_origin[~Mask]
            types_new = types_new.cpu()
            types_new = types_new.numpy()
            types_new = np.array(types_new)
            idx = np.where(types_new == 1)[0]
            idx = idx.tolist()
            normals[idx] = normals_new[idx]
    ...
    def compute_level_surface_points_from_camera(   ### coarse中用到了get_normal
        self, 
        nerf_cameras=None,
        cam_idx=0,
        rasterizer=None,
    ...
    def compute_level_surface_points_from_camera_efficient(  ###使用getnormal
        self, 
        nerf_cameras=None,
        cam_idx=0,
        rasterizer=None,
    ...
    def compute_level_surface_points_from_camera_fast(  ## 用到了get_normal
        self, 
        nerf_cameras=None,
        cam_idx=0,
        rasterizer=None,
