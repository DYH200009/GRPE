...
#define THETA_THRESHOLD 0.004       // 5度
...
//     std::function<char*(size_t)> binningFunc = resizeFunctional_(binningBuffer);
//
//     char* size_0 = nullptr;
//     BinningState::fromChunk(size_0, P);
//     size_t binning_chunk_size = ((size_t)size_0) + 128;
//
//
// 	char* binning_chunkptr = binningFunc(binning_chunk_size);
//     BinningState binningState = BinningState::fromChunk(binning_chunkptr, P);
//
//      // Initialize binning memery
//      cudaMemcpy(binningState.point_list_keys_unsorted, xyz_ids.contiguous().data<int>(), P * sizeof(int), cudaMemcpyDeviceToDevice);
//      initIndexCUDA<<<(P + 255) / 256, 256>>>(P, binningState.point_list_unsorted);
//
...
//     char* binning_buffer_ptr = reinterpret_cast<char*>(binning_buffer.contiguous().data_ptr());
//     BinningState binningState = BinningState::fromChunk(binning_buffer_ptr, P);
//
//     processBackwardCUDA<<<(P + 255) / 256, 256>>>(P,
//                                                 binningState.point_list_keys,
//                                                 binningState.point_list,
//                                                 xyzs.contiguous().data<float>(),
//                                                 xyz_ids.contiguous().data<int>(),
